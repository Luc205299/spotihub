...
function Auth() {
    ...
    useEffect(() => {
        ...
        const fetchAccessToken = async () => {
            if (!code) {
                ...
                redirect(clientId, code);
            }
        };

        fetchAccessToken();
    }, []); // Ne dépend d'aucune valeur, donc cela ne devrait être exécuté qu'une seule fois
    ...
    async function redirect(clientId, code) {
        const accessToken = await getAccessToken(clientId, code);
        localStorage.setItem('accessToken', accessToken);
        navigate('/accueil'); // Rediriger vers la page d'accueil après avoir obtenu le token
    }
    ...
    return (
        <div className="loading">Communicating with Spotify...</div>
    );
}
